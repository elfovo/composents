...
interface ModernLayoutNavProps {
  layoutDemos: LayoutDemo[];
  activeDemo: string;
  setActiveDemo: (id: string) => void;
}

export default function ModernLayoutNav({ 
  layoutDemos, 
  activeDemo, 
  setActiveDemo 
}: ModernLayoutNavProps) {
  const navItems = layoutDemos.map((demo: LayoutDemo) => ({
    label: demo.name,
    href: '#'
  }));
    ...
    <div className="flex justify-center mb-12">
      <div className="relative" ref={containerRef}>
        <GooeyNav
          ...
          initialActiveIndex={layoutDemos.findIndex((demo: LayoutDemo) => demo.id === activeDemo)}
        />
        <style jsx>{`
          :root {
            --color-1: #5239ff;
