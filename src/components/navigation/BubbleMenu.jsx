...
export default function BubbleMenu({
  ...
  return (
    <>
      <nav ref={menuRef} className={containerClassName} style={style} aria-label="Main navigation">
        <div className="bubble logo-bubble" aria-label="Logo" style={{ background: menuBg }}>
          <span className="logo-content">
            {typeof logo === 'string' && logo.includes('.svg') ? (
              <img src={logo} alt="MonDev Logo" className="bubble-logo" style={{ width: '24px', height: '24px' }} />
            ) : typeof logo === 'string' ? (
              <span className="logo-text" style={{ color: menuContentColor, fontWeight: 'bold', fontSize: '16px' }}>
                {logo}
              </span>
            ) : (
              logo
            )}
            <span className="logo-text" style={{ color: menuContentColor, marginLeft: '8px', fontWeight: 'bold', fontSize: '16px' }}>
              MONDEV
            </span>
          </span>
        </div>
