...
interface ModernAnimationNavProps {
  animationDemos: AnimationDemo[];
  activeDemo: string;
  setActiveDemo: (id: string) => void;
}

export default function ModernAnimationNav({ 
  animationDemos, 
  activeDemo, 
  setActiveDemo 
}: ModernAnimationNavProps) {
  const navItems = animationDemos.map((demo: AnimationDemo) => ({
    label: demo.name,
    href: '#'
  }));
    ...
    <div className="flex justify-center mb-12">
      <div className="relative" ref={containerRef}>
        <GooeyNav
          ...
          initialActiveIndex={animationDemos.findIndex((demo: AnimationDemo) => demo.id === activeDemo)}
        />
        <style jsx>{`
          :root {
            --color-1: #5239ff;
